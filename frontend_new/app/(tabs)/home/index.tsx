...
export default function HomeScreen() {
  ...
  const panGesture = Gesture.Pan()
    ...
    .onEnd((e) => {
      if (mode.value === "feed" && e.translationX > SWIPE_THRESHOLD) {
        ...
        feedX.value = withTiming(mode.value === "saved" ? SAVED_WIDTH : 0, {
          duration: 220,
        });
        searchX.value = withTiming(mode.value === "search" ? 0 : width, {
          duration: 220,
        });
      }
      hapticOnce.value = false;
  ...
  const feedDimStyle = useAnimatedStyle(() => ({
    opacity: interpolate(
      Math.max(feedProgress.value, searchProgress.value),
      [0, 1],
      [0, 0.4],
      Extrapolate.CLAMP,
    ),
